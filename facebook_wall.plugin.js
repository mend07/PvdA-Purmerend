	...
	$.fn.facebook_wall = function(options) {
		...
		options = $.extend({
			...
			text_labels: {
				...
				comments: {
					singular: '% keer gereageerd',
					plural: '% keer gereageerd'
				},
				like: 'Like',
				comment: 'Reageer',
				share: 'Deel',
		...
		function timeToHuman(time) {
			...
			if (time_difference < 10) {
				return 'paar seconde geleden';
			} else if (time_difference < 60) {
				return Math.round(time_difference) + ' seconden geleden';
			} else if (Math.round(time_difference/60) === 1) {
				return Math.round(time_difference/60) + ' minuut geleden';
			} else if (Math.round(time_difference/60) < 60) {
				return Math.round(time_difference/60) + ' minuten geleden ago';
				...
				return Math.round(time_difference/(60*60)) + ' uur geleden';
			} else if (Math.round(time_difference/(60*60*24)) === 1) {
				return Math.round(time_difference/(60*60*24)) + ' dagen geleden ago';
			} else if (Math.round(time_difference/(60*60*24)) <= 10) {
				return Math.round(time_difference/(60*60*24)) + ' dagen geleden';
			} else {
